...
export const handle: Handle = async ({ event, resolve }) => {
  ...
  try {
    ...
    if (shouldLogRequest) {
      // Log raw HTTP request for all applicable types
      await logRawHttpRequest(
        request.clone(),
        isAuthRequest // Still pass isAuth to potentially omit sensitive body
      );
    
      // Log standard request info
      logRequest(request, response.status, username);

      // Log raw HTTP response for all applicable types
      await logRawHttpResponse(
        request, // Pass original request for context
        response.clone(),
        isAuthRequest // Still pass isAuth to potentially omit sensitive body
      );
    }
